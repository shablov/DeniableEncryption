...
//		Logger::toSyslog(Event::SL_3_Error, error);
		return;
	}
	...
	{
		clearContent();
//		Logger::toSyslog(Event::SL_3_Error, "Корневой тэг xml документа неправильный.");
		return;
	}
}
...
void DictPrivate::check()
{
	if (duplicated())
	{
		mDoc.clear();
		mRoot.clear();
//		Logger::toSyslog(Event::SL_3_Error, "Xml документ неправильный - есть дублирующиеся тэги.");
	}
}
...
QByteArray DictPrivate::dictByteArrayFromFile()
{
	...
	if (!file.exists())
	{
//		Logger::toSyslog(Event::SL_3_Error, "Файл словаря не существует.");
		return QByteArray();
	}
	if (!file.open(QIODevice::ReadOnly))
	{

//		Logger::toSyslog(Event::SL_3_Error, "Файл словаря не может быть открыт. Ошибка = " + file.errorString());
		return QByteArray();
	}
...
{
//	Logger::toSyslog(Event::SL_3_Error, "Дублируйщий код в строке = " +
